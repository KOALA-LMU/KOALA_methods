...
insufficiently addressed or  not covered at all. Furthermore, main interest
...
until election day (``fore-cast''). Since our method is based on the posterior distribution
of party shares, our method can be applied for a variety of questions related to the
...
\keywords{\red{4 to 6 keywords} Election analysis \and Opinion polls \and Election reporting \and Multinomial-Dirichlet}
...
A key role played the FDP, which had to reach at least the minimum voter share of
$5\%$ to get into parliament and make a majority possible. 
Figure \ref{fig:2013} summarizes the reported party
...
To visualize the results for specific events
we use density plots (see also Fig. \ref{fig:seatDist}).
These are well suited to communicate both
probability estimates and the uncertainty underlying opinion polls.
To depict the {\it development} of such probabilities
for a specific event, e.g., a coalition majority or a party passing the 5\% threshold,
we recommend extending the
visualization of one density by using so-called
\emph{ridgeline plots} \citep{wilke_2017} that visualize the development of such densities.
In Figure~\ref{fig:2013_fdp} this plot type and the development of the POEs
is compared to the observed, redistributed shares of FDP, which
...
therefore FDP only enters the parliament in about $50\%$ of the simulations.
...
in Figure~\ref{fig:seatDist_time}, it can again be seen that even small changes
in the redistributed voter share can make an immense difference regarding the
probability of the coalition to obtain a majority.
Especially regarding the months preceding election day this lead to heavily
fluctuating POE's based on the Forsa opinion polls. \\
...
As discussed in the context of Figure~\ref{fig:2013_fdp}, the effect of changes in voter share on
the POE strongly depends on the base level of the voter share.
E.g., a rise in voter share from $45\%$ to $48\%$ in March 2013 only had
very small impact on the corresponding POE as a seat majority was still very improbable.
Instead, dealing with voter shares near $50\%$, even very small changes heavily influence
the event probabilities.
Secondly, probabilities do not only take into account
joint party shares and sample uncertainty,
...
The impact of the uncertainty regarding FDP entering the parliament is
...
below $48\%$ correspond to very small POEs
...
%Note that we only perform now-casting of the POE analyzing the current
...
% and to avoid misinterpretation because of similarity to existing plots.
